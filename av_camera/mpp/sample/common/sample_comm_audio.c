...
void *SAMPLE_COMM_AUDIO_AiProc(void *parg)
{
    ...
    while (pstAiCtl->bStart)
    {
        ...
        if (FD_ISSET(AiFd, &read_fds))
        {
            ...
            printf("--%s pstAiCtl->bSendAenc=%d\n",__FUNCTION__,pstAiCtl->bSendAenc);
                                 ...
                                 AIO_ATTR_S* pstAioAttr, AUDIO_SAMPLE_RATE_E enOutSampleRate, HI_BOOL bResampleEn, AI_VQE_CONFIG_S* pstAiVqeAttr)
{
    ...
    printf("%s: pstAioAttr->u32ClkChnCnt = %d, s32AiChnCnt: %d\n", __FUNCTION__, pstAioAttr->u32ClkChnCnt, s32AiChnCnt);
    ...
    {
        chn = i;
        s32Ret = HI_MPI_AI_EnableChn(AiDevId, chn);
        if (s32Ret)
        {
            printf("%s: HI_MPI_AI_EnableChn(%d,%d) failed with %#x\n", __FUNCTION__, AiDevId, chn, s32Ret);
            return s32Ret;
        }

        if (HI_TRUE == bResampleEn)
        {
            s32Ret = HI_MPI_AI_EnableReSmp(AiDevId, chn, enOutSampleRate);
            if (s32Ret)
            {
        ...
        {
            s32Ret = HI_MPI_AI_SetVqeAttr(AiDevId, chn, SAMPLE_AUDIO_AO_DEV, chn, pstAiVqeAttr);
            if (s32Ret)
            {
            ...
            s32Ret = HI_MPI_AI_EnableVqe(AiDevId, chn);
            if (s32Ret)
            {
