...
HI_S32 SAMPLE_NVP6114_CfgV(VIDEO_NORM_E enVideoMode, SAMPLE_VI_6124_MODE_E enViMode)
{
    int fd, i;
    int video_mode;
    ...
    /* BEGIN: Modified by zhangjinlei, 2016/4/21  DES:调试3521A视频VIVO功能   PN:16042001001 */
    int chip_cnt = 1;
    /* END:   Modified by zhangjinlei, 2016/4/21   PN:16042001001 */

    fd = open(NVP6114_FILE, O_RDWR);
    if (fd < 0)
    {
        printf("open nvp6114 (%s) fail\n", NVP6114_FILE);
        return -1;
    }
    ...
    switch(enViMode)
    {
    	case SAMPLE_VI_MODE_6124_960H:
			for(i=0;i<chip_cnt*4;i++)
			{
				adjust.chip = 0;
                adjust.mode = 0;
				ioctl(fd, IOC_VDEC_SET_VIDEO_MODE, &adjust);
			}
            #if 0
			for(i=0;i<chip_cnt;i++)
			{
...
HI_S32 SAMPLE_COMM_VI_ADStart(SAMPLE_VI_MODE_E enViMode, VIDEO_NORM_E enNorm)
{
    ...
    switch (enViMode)
    {
        ...
        case SAMPLE_VI_MODE_8_720P:
            ...
            /* BEGIN: Modified by zhangjinlei, 2016/4/21  DES:调试3521A视频VIVO功能   PN:16042001001 */
            //s32Ret = VI_MST_NVP6124_CfgV(enNorm, enMode);
            s32Ret = SAMPLE_NVP6114_CfgV(enNorm, enMode);
            /* END:   Modified by zhangjinlei, 2016/4/21   PN:16042001001 */
            if (s32Ret != HI_SUCCESS)
            {
