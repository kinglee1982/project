...
public:
    ...
    explicit ImsIOBuffer(size_t size):
        m_ptr(NULL),
        m_length(0),
        m_readPos(0),
        m_writePos(0),
    ...
    ImsIOBuffer(const ImsIOBuffer& ref):
        m_ptr(NULL),
        m_length(0),
        m_readPos(0),
        m_writePos(0),
    ...
    const ImsIOBuffer& operator = (const ImsIOBuffer& ref)
    {
        assign(ref);
        return (*this);
    }
    ...
    size_t writeFrom(const ImsIOBuffer& buf, size_t length)
    {
    ...
    size_t writeFrom(const ImsIOBuffer& buf)
    {
        return write(buf.getReadPtr(), buf.readable());
    }

    size_t moveFrom(ImsIOBuffer& buf, size_t length)
    {
...
protected:
    ...
    void assign(const ImsIOBuffer& ref)
    {
        expect(ref.m_length);
